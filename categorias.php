...
<head>
    <meta charset="UTF-8">
    <title>Categorias - NexusFlow</title>
...
<div class="container" style="display:flex;">
    ...
    <div class="conteudo">
        ...
        <div class="card-erp" style="max-width:500px;">
            <h3 style="margin-bottom:15px;">Nova Categoria</h3>
            <?php if ($erro): ?><p style="color:#dc2626;"><?= htmlspecialchars($erro) ?></p><?php endif; ?>
            <form method="post" style="display:flex; gap:10px;">
                <?php csrf_field(); ?>
                <input type="text" name="nome" class="input-erp" placeholder="Nome da categoria" required style="flex:1;">
                <button type="submit" name="nova_categoria" value="1" class="btn-primary">Adicionar</button>
            </form>
        </div>
                ...
                <table class="table-erp">
                    ...
                    <tbody>
                        <?php if ($res->num_rows > 0): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="3" class="empty-state">Nenhuma categoria cadastrada.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
<div id="modalEdicao" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.4); align-items:center; justify-content:center; z-index:9999;">
    <div style="background:#fff; padding:25px; border-radius:12px; width:100%; max-width:400px;">
        <h3 style="margin-bottom:15px;">Editar Categoria</h3>
